...
def label_text_and_polygon_dataframes(
    ...
    for i, _ in enumerate(unique_non_noise_labels):
        ...
        if priority is not None:
            size_factor = np.mean(priority[cluster_mask])
        else:
            size_factor = 1.0
        cluster_sizes.append((np.sum(cluster_mask) ** 0.25) * size_factor)
...
def render_html(
    ...
    # Check if node_connections exists and append connections
    if node_connections is not None:
        point_dataframe["connections"] = node_connections
        point_data_cols.append("connections")
    else: 
      print("No node Connections!")
    ...
    if inline_data:
        ...
        if edge_data is not None and not edge_data.empty:
          buffer = io.BytesIO()
        ...
        base64_node_connections = None
        if node_indices is not None:
            json_bytes = json.dumps(node_indices).encode()
            gzipped_bytes = gzip.compress(json_bytes)
            base64_node_indices = base64.b64encode(gzipped_bytes).decode()
        else:
            base64_node_indices = None
        if enable_histogram:
    ...
    html_str = template.render(
        ...
        **histogram_ctx,
        ...
        base64_node_connections=base64_node_connections,
        base64_node_indices=base64_node_indices,
        ...
        edge_color="[100, 100, 100, 128]",
        edge_width_min_pixels=1,
        edge_width_max_pixels=15,
        edge_pickable="false",
        data_bounds=bounds,
